...
svg(file="plots/Topics_network_recoded.svg", width = 8, height= 6)
...
# Setting new graphic parameters
 par(mar = c(0,0,1.2,0), usr = c(-1.2,1.2,-1.2,1.2)) # -1.8,1.8,-1.8,1.8 ## xmin, xmax, ymin, et ymax
...
## extract the component including PNA communications (draft to be edited - the issue is that PNA has two distinct indid (62 and 3620),
...
vert_ids_2 <- V(g)[components$membership == (V(g)$membership[V(g)$name == "693"])]

# subgraph
cg <- igraph::induced_subgraph(g, vert_ids)
cg2 <- igraph::induced_subgraph(g, vert_ids_2)

cg <- cg + cg2
cg <- add_edges(cg, c(1,4, 2,4, 3,4))

V(cg)$rtopic <- c("Biomass conversion", "Biomass conversion", "New technologies", "Biomass conversion", "Biomass conversion")
V(cg)$year <- c("2017", "2017", "2019", "2015", "2015")
